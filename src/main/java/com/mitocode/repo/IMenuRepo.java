...
public interface IMenuRepo extends JpaRepository<Menu, Integer>{
	...
	@Query(value="select m.* from menu_rol mr inner join usuario_rol ur on ur.id_rol = mr.id_rol inner join menu m on m.id_menu = mr.id_menu inner join rol r on r.id_rol = ur.id_rol where r.nombre = :nombre", nativeQuery = true)
	List<Object[]> listarMenuPorRol(@Param("nombre") String nombrerol);
